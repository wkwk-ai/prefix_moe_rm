#!/bin/bash
# GRPO training with Hybrid Reward Manager
# Uses mixed data (Ernie-rlhf open-ended + MedKgQA MCQ) with cognitive-level-adaptive hybrid reward.
#
# Prerequisites:
#   1. prepare_medkgqa_for_grpo.py + classify_medkgqa_with_router.py (Phase 1)
#   2. RM fine-tuned on CoT preference pairs (Phase 2)
#   3. prepare_mixed_data.py (Phase 3)
set -x
...
# Use the CoT-extended RM (after fine-tuning on CoT preference pairs)
...
# Mixed training data (Ernie-rlhf + MedKgQA)
...
# ---------- Data preparation ----------
# Prepare MedKgQA data if not exists
MEDKGQA_PARQUET=/home/haowang/prefix_moe_rm/prepare_data/medkgqa/medkgqa-train.parquet
if [ ! -f "$MEDKGQA_PARQUET" ]; then
    echo "Preparing MedKgQA data..."
    python3 examples/grpo_trainer/prepare_medkgqa_for_grpo.py \
        /home/haowang/Fleming-R1/data/MedKgQA_en.jsonl \
        "$MEDKGQA_PARQUET"

    echo "Classifying MedKgQA with router..."
    python3 examples/grpo_trainer/classify_medkgqa_with_router.py \
        --parquet "$MEDKGQA_PARQUET" \
        --router_ckpt "$ROUTER_CKPT_PATH" \
        --output "$MEDKGQA_PARQUET"
fi

# Prepare Ernie-rlhf parquet if not exists
...
# Prepare mixed data if not exists
...
# ---------- Launch GRPO with Hybrid Reward ----------
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_grpo_hybrid_reward' \
    trainer.experiment_name='qwen2_1.5b_hybrid_mcq_openqa' \
    ...
    trainer.rollout_data_dir=/home/haowang/prefix_moe_rm/verl/rollout_outputs_hybrid \
    trainer.validation_data_dir=/home/haowang/prefix_moe_rm/verl/validation_outputs_hybrid \
    trainer.benchmark_data_dir=/home/haowang/prefix_moe_rm/verl/benchmark_outputs_hybrid \
