...
def eval_rm(innerMoERM, tokenized_dataset):
    ...
    with torch.no_grad():
        ...
        for index in tqdm(random_numbers):
            ...
            attention_mask_list = tokenized_data['attention_mask']
            rank = tokenized_data['rank']
            cat_probs = tokenized_data['cat_probs'].to(config.device)

            rewards = []
            for idx, input_ids in enumerate(input_ids_list):
                input_ids = input_ids.to(config.device)
                attention_mask = attention_mask_list[idx].to(config.device)
                reward = innerMoERM(input_ids, attention_mask, cat_probs)
                rewards.append(reward)
            ...
            for ith, rank1 in enumerate(rank):
                for jth, rank2 in enumerate(rank):
                    ...
                    reward1 = rewards[ith]
                    reward2 = rewards[jth]
                    if rank1 > rank2 and reward1 > reward2 or rank1 < rank2 and reward1 < reward2:
                        acc += 1
